...
paraphrased: the system prompt is the exact text the model receives,
generated by running the extension's own prompt builder.
...
Sessions are not persisted. Reviews run on the developer's own machine
against their own Claude account; only the redacted report is uploaded.
...
change — 15159 characters.

Sections included: ruleset, cross-cutting/architecture,
cross-cutting/security, cross-cutting/performance, lang/typescript.
